...
4. [Pick a suitable colour column](https://docs.microreact.org/instructions/labels-colours-and-shapes#colours) to change what the colours of the nodes are representing (e.g. Serotype)
   > In the GPS Project, we have standardised colour palette for [GPSCs](https://github.com/GlobalPneumoSeq/gps-database-processor/blob/main/scripts/data/gpsc_colours.csv) and [serotypes](https://github.com/GlobalPneumoSeq/gps-database-processor/blob/main/scripts/data/serotype_colours.csv). You can add a `<FIELD NAME>__colour` column (e.g. `GPSC__colour`) in your metadata (`results_qcpass.csv` in this case) to specify the colour palette of a column. 
